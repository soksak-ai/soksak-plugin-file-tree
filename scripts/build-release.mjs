#!/usr/bin/env node
// The release build, for any plugin — id, files and deps all read from the manifest.
// It enforces only what every unit must have, and hardcodes nothing about this one.
...
// The invariants every unit shares, and no more
...
// FILES = the standard candidates that exist; main.js and plugin.json are required
